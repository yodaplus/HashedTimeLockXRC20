...
/**
* @title Hashed Timelock Contracts (HTLCs) on XDC XRC20 tokens.
*
* This contract provides a way to create and keep HTLCs for XRC20 tokens.
*
...
contract HashedTimelockXRC20 {
    ...
    event HTLCXRC20Withdraw(bytes32 indexed contractId);
    event HTLCXRC20Refund(bytes32 indexed contractId);
    ...
    struct LockContract {
        ...
        uint256 amount;
        bytes32 hashlock;
        // locked UNTIL this time in seconds.
        uint256 timelock;
        bool withdrawn;
        bool refunded;
        bytes32 preimage;
    ...
    modifier tokensTransferable(address _token, address _sender, uint256 _amount) {
        ...
        require(
            XRC20(_token).allowance(_sender, address(this)) >= _amount,
     ...
     *
     * NOTE: _sender must first call approve() on the token contract.
     ...
     * @param _tokenContract XRC20 Token contract address.
        ...
        returns (bytes32 contractId)
    {
        ...
        if (!XRC20(_tokenContract).transferFrom(msg.sender, address(this), _amount))
    ...
    * @return bool true on success
        ...
        returns (bool)
    {
        ...
        c.withdrawn = true;
        XRC20(c.tokenContract).transfer(c.receiver, c.amount);
        emit HTLCXRC20Withdraw(_contractId);
        return true;
    }
        ...
        returns (bool)
    {
        LockContract storage c = contracts[_contractId];
        c.refunded = true;
        XRC20(c.tokenContract).transfer(c.sender, c.amount);
        emit HTLCXRC20Refund(_contractId);
        return true;
    }
